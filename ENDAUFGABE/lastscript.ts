...
const boo: HTMLElement = document.querySelector("#boobutton"); // speichert den boobutton
const laugh: HTMLElement = document.querySelector("#laughbutton"); // speichert den laughbutton
const duck: HTMLElement = document.querySelector("#duckbutton"); // speichert den duckbutton
const boing: HTMLElement = document.querySelector("#boingbutton"); // speichert den boingbutton
const applause: HTMLElement = document.querySelector("#applausebutton"); // speichert den applausebutton
...
const soundButtons: Array<HTMLElement> = [boo, laugh, duck, boing, applause];
...
onButton.addEventListener('click', function(oEv) { // Auf das click event des on buttons wird eine anyonyme function registriert, d.h der nachfolgende code wird ausgeführt sobald der user die on checkbox anklickt
  if (onButton.checked == true) { // prüfen ob der user den hacken gesetzt oder entfernt hat 
    on = true; // falls ja: on variable auf true setzen Erinnerung: die on variable (boolean) gibt an, ob das programm an ist und ob der user die sound buttons benutzen kann
    turnCounter.innerHTML = "-"; // Setzt den Text des html elements auf -  Erinnerung: das html element ist die count anzeige, siehe turnCounter variable oben
  } else { //falls nein:
    on = false; // setze on variable auf false, Effekt: user kann keine sound buttons benutzen
    turnCounter.innerHTML = ""; // Enferne den Text des html elements
    clearInterval(intervalId); // Stoppt weitere Wiederholungen der setInterval Funktion, die später auftaucht
  }
});
...
boo.addEventListener('click', (event) => {  // wir registrieren auf das click event des boo buttons folgenden code 
  if (on) { // prüfen, ob die variable on auf true ist (Erinnerung: die on Variable gibt hier an, ob der User derzeit buttons betätigen kann), falls ja:
